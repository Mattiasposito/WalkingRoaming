...
def router(filepath):
    ...
    disconnectionFlag = "SCANNING -> DISCONNECTED"

    if "datalogic" in endfile.lower() or "honey" in endfile.lower():
        print("datalogic or honey file ..")
        disconnectionFlag = "COMPLETED -> DISCONNECTED"
    ...
    with open(filepath, 'r') as file:
        ...
        for index, (currentLine, nextLine) in enumerate(zip(lines, islice(lines, 1, None))):
            ...
            if currentLine.split()[1] == "15:09:09.809":
                print("ecco")
            element["timestamp"] = disconnectTimestamp
            # Check if it is a disconnection
            if disconnectionFlag in currentLine:
...
if __name__ == '__main__':
    ...
    # /Users/automationnapoli/Downloads/wifi_datalogic_2.4_bad_coverage_filtered.log
    path = 'wifi_honey_triband_good_coverage_busy_filtered.log'
    results = router(path)
    ...
    for event in eventList:
        ...
        if event["type"] == "DISCONNECT":
            ...
            YaxisValue = int(event["RSSI"]) if "RSSI" in list(event.keys()) else -100
            colors.append('g')
        ...
        events.append(
            {
                ...
                "frequency_before": event["before"] if "before" in list(event.keys()) else "",
                "frequency_after": event["after"] if "after" in list(event.keys()) else "",
                "YaxisValue": int(YaxisValue),
                "label": str(timestamp),
                "bssID": str(event["bssID"]) if event["type"] != "DISCONNECT" else "",
                "logTimestamp": str(event["timestamp"]),
                "RSSI": str(event["RSSI"]) if "RSSI" in list(event.keys()) else -100,
                "INTRAROAMING": event["INTRAROAMING"] if "INTRAROAMING" in list(event.keys()) else False
            }
        )
    ...
    for index, event_type in enumerate(events):
        color = 'r'
        if event_type["type"] == "ROAMING" and event_type["INTRAROAMING"]:
            color = "y"
    ...
    for singleEvent in events:
        ...
        if singleEvent["frequency_before"] != "":
            ...
            beforeConnectionType = "Not Found"

        # Frequency after event ..
        afterConnectionType = "2.4GHz"

        if singleEvent["frequency_after"] != "":
        ...
        elif str(singleEvent["type"]) == "4WAY_HANDSHAKE" and singleEvent["INTRAROAMING"]:
            intraroaming_counter = intraroaming_counter + 1
            roaming_counter = roaming_counter + 1
